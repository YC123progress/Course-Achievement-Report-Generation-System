...
def safe_read_csv(filename, encoding='utf-8'):
    """安全读取CSV文件，支持多种编码，并尽早过滤空行空列以降低内存峰值"""
    ...
    for enc in encodings:
        try:
            ...
            print(f"[OK] 成功读取 {filename} (编码: {enc}, 行数: {len(df)}, 列数: {len(df.columns)})")
            return df
        except Exception as e:
            if enc == encodings[-1]:
...
def read_all_csv(config):
    ...
    for i, df in enumerate(dfs[1:], 1):
        if not df.empty:
            merged_df = pd.merge(merged_df, df, on=['学号', '姓名'], how='outer', copy=False)
